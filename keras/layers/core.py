...
class Dropout(Layer):
    ...
    def __init__(self, rate, noise_shape=None, seed=None, training=False, **kwargs):
        super(Dropout, self).__init__(**kwargs)
        self.rate = min(1., max(0., rate))
        self.noise_shape = noise_shape
        self.seed = seed
        self.supports_masking = True
        self.train_only = not training
    ...
    def call(self, inputs, training=None):
        if 0. < self.rate < 1.:
            ...
            in_training = training if self.train_only == True else True
            def dropped_inputs():
